...
func (implementation *RepositoryQuestionImplementation) Create(ctx context.Context, tx *sql.Tx, model models.Question) (models.Question, error) {
	...
	result, err := tx.ExecContext(ctx, query, model.Name, model.Language, model.Chapter, model.Username, model.Amount, model.GeminiFileURI, model.FileName, model.Result)
	if err != nil {
		return model, err
	}
...
func (implementation *RepositoryQuestionImplementation) GetById(ctx context.Context, tx *sql.Tx, id int) (models.Question, error) {
	query := fmt.Sprintf("SELECT id, name, language, chapter, amount, file_name, result FROM %s WHERE id = ? LIMIT 1", models.QuestionTable)
	...
	if rows.Next() {
		err = rows.Scan(&model.Id, &model.Name, &model.Language, &model.Chapter, &model.Amount, &model.FileName, &model.Result)
		if err != nil {
			return model, err
		}
	} else {
...
func (implementation *RepositoryQuestionImplementation) GetByIdWithExams(ctx context.Context, tx *sql.Tx, id int) (models.Question, error) {
	query := fmt.Sprintf("SELECT q.id, q.name, q.language, q.chapter, q.amount, q.file_name, q.result, e.id, e.username, e.score FROM %s q LEFT JOIN %s e ON q.id = e.question_id AND e.exam_at IS NOT NULL WHERE q.id = ?", models.QuestionTable, models.ExamTable)
	...
	for rows.Next() {
		...
		err = rows.Scan(&rowDataQuestion.Id, &rowDataQuestion.Name, &rowDataQuestion.Language, &rowDataQuestion.Chapter, &rowDataQuestion.Amount, &rowDataQuestion.FileName, &rowDataQuestion.Result, &rowDataExam.Id, &rowDataExam.Username, &rowDataExam.Score)
